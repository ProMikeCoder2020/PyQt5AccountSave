...
from Encryption_System import encode_string
...
class Window(QMainWindow):
    ...
    def init_ui(self):  # initalize the homepage Ui and set its basic properties
        ...
        self.account_label = QtWidgets.QLabel(self.central_frame)
        self.account_label.setText("Never again forget your account info!!! ")
        self.account_label.setStyleSheet('color:red')
        self.account_label.move(60, 25)
        self.account_label.setFont(self.big_font)
        self.account_label.adjustSize()
        if MyAccount.get_current_account():
            print("login")
            self.login()
    ...
    def create_account_info(self):
        ...
        self.button_change_get_account_info = QtWidgets.QPushButton("Manage account info.", self)
    ...
    def get_account_info_widget(self):
        ...
        self.choose_account_type_to_view.currentTextChanged.connect(lambda:
                                                                    self.update_filter(self.choose_account_type_to_view.currentText()))
    ...
    def login(self):
        ...
        self.account_label.setText(MyAccount.currentAccount.username)
        self.account_label.adjustSize()
    ...
    def log_out(self):
        ...
        self.login_button.clicked.disconnect()
        self.login_button.clicked.connect(lambda: self.login_widget_setup("login"))
        self.account_label.setText("Never forget again your account info!!!")
        self.account_label.adjustSize()
        with open("current_account.txt", "w"):
            pass
    ...
    def store_new_account_info(self):
        AccountSave(self, self.account_name.text(), self.account_password.text(), self.account_username.text(),
    ...
    def change_account_type(self, account_type):
        if account_type == "Web App":
            self.choose_account_type.setCurrentIndex(self.choose_account_type.findText
                                                     ("Web App", QtCore.Qt.MatchFixedString))
            self.account_name_label.setText("Account Name")
            self.account_username_label.setText("Username")
            self.account_password_label.setText("Password")
            self.submit_new_account.setText("Store account info")
            ...
            self.choose_account_type.setCurrentIndex(self.choose_account_type.findText
                                                     ("Padlock", QtCore.Qt.MatchFixedString))
    ...
    def get_account_info(self):
        ...
        self.choose_account_type_to_view.setCurrentIndex(self.choose_account_type_to_view.findText
                                                         ("All", QtCore.Qt.MatchFixedString))
    ...
    def change_account_info(self):
        try:
            AccountSave.change_account_info(self, self.choose_account.currentText(),
                                            [encode_string(self.account_name_get.text(), MyAccount.currentAccount.key),
                                             encode_string(self.account_password_get.text(), MyAccount.currentAccount.key),
                                             encode_string(self.account_username_get.text(), MyAccount.currentAccount.key)])
        except ValueError:
            pass
        self.reset_choose_account_list()
